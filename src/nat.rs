...
fn is_direct(path: &Path) -> bool {
    matches!(path, Path::DirectDialed | Path::DirectPunched)
}
...
fn nat_cfg(policy: ReservationPolicy, relay: Option<minip2p::PeerAddr>, force_relay: bool) -> NatConfig {
    let mut c = NatConfig {
...
fn nat_builder(policy: ReservationPolicy) -> minip2p::EndpointBuilder {
    Endpoint::builder()
        .agent_version(AGENT)
        .protocol(ECHO_PROTOCOL)
        .nat_config(nat_cfg(policy, None, false))
}
...
fn finish_ok(
    ...
    DialResult {
        ...
        error: if ok {
            ...
            Some(format!("{note}; lost {lost} frames (sent={sent} recv={received})"))
        },
        dial_ms: path_ms,
        identify_ms: path_ms,
        echo_open_ms: 0,
...
fn echo_n(
    ...
    while received < n {
        ...
        if sent < n && outstanding.is_empty() {
            sent += 1;
            let frame = encode_header(sent, t0.elapsed().as_millis() as u64);
            let len = frame.len() as u64;
            eps[init]
                .send_stream(&resp_peer, stream, frame)
                .map_err(|e| format!("send_stream seq {sent}: {e}"))?;
            bytes_sent += len;
        ...
        for ev in &step[resp] {
            ...
            {
                eps[resp]
                    .send_stream(&init_peer, resp_stream, data.clone())
                    .map_err(|e| format!("responder echo send: {e}"))?;
            }
        }
        for ev in &step[init] {
            if let Event::StreamData {
...
fn run_direct(
    ...
    let result = (|| -> Result<DialResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut a = bind(nat_builder(ReservationPolicy::Always), transport)?;
        let mut b = bind(nat_builder(ReservationPolicy::Always), transport)?;
        let b_addr = b.listen()?;
        a.listen()?;
        ...
        if !eps[0].path(&b_peer).is_some_and(|p| is_direct(&p)) {
    ...
    eprintln!(
        ...
        r.ok, r.sent, r.received, r.lost, r.avg_echo_rtt_us(), r.wall_ms, r.error.clone().unwrap_or_default()
    );
    r
}
...
fn run_nopath(
    ...
    eprintln!(
        ...
        r.ok, r.wall_ms, r.error.clone().unwrap_or_default()
    );
    r
}
...
/// NAT / circuit scenarios. Circuit is a real failure if the hop cannot bridge.
